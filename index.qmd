---
title: "**<b style = 'color : #E34B2A;'>Plantilla de códigos</b>**"
subtitle: 'POL278'
date: "2024-8-3"
...
describe(data$q2) # Percentiles y valores altos y bajos, para sacar outliers. 
```
...
p1 <- ggplot(datalapop, aes(x = "", y = psc8, color = "")) +
  ...
  theme(legend.position = "top", axis.text.y = element_blank(), 
        panel.background = element_rect(fill = "white", colour = "white")) + 
  geom_jitter(shape = 16, position = position_jitter(0.2)) + 
  labs(title = "Edad de los encuestados", x = "", y ="Index", 
       subtitle = "", caption = "ENDO 2020")
ggplotly(p1)
...
ggplot(datalapop, aes(x = q2, color = "")) + 
  ...
  # agregar bins = 30 o breaks=seq()
    labs(title ="Edad de los encuestados", y = "", x="Index", subtitle = "", 
         caption = "ENDO 2020")+
...
```
  - PIE
```markdown
pie(grafico1, col = rainbow(7), main = "Índice de democracia", radius = 1, 
    cex = 1.2)
```
...
boxplot(data$State_Legit ~ data$Type) 
# ~ --> indica graficar la variable (State_Legit) segun los grupos de (Type)
```
  - HISTOGRAMA
```markdown
...
data$var=factor(data$var,
                 levels = levels(data$var),
                 labels = c("Muy en desacuerdo","Algo en desacuerdo", 
...
```markdown
data$P4_9_1_A_B=car::recode(data$P4_9_1_A_B,"1=4; 2=3; 3=2; 4=1") 
# recodificas solo si es necesario, mirar el despliegue de tus variables.
...
data$xeno = data$P4_9_1_A_A+data$P4_9_1_A_B+data$P4_9_1_A_C+data$P4_9_1_A_D + 
data$P4_9_1_A_E
```
...
p1 <- ggplot(datalapop, aes(x = "", y = psc8, color ="")) + 
  ...
  theme(legend.position = "top", axis.text.y = element_blank(), 
        panel.background = element_rect(fill = "white", colour = "white")) + 
  geom_jitter(shape = 16, position = position_jitter(0.2)) + 
  labs(title = "Edad de los encuestados", x = "", y ="Index", subtitle = "", 
       caption = "ENDO 2020")
ggplotly(p1)
...
Aquí recodífico
```markdown
SI = data1[data1$P4_5 == "Si", "xeno"] 
# variable xeno es el indicador (númerico) y P4_5 es la cualitativa.
...
```markdown
library(car)
leveneTest(lapop$var1, lapop$var2) 
# Prueba de Levene, Pr es igual al Pvalue, es decir, varianzas iguales. 
```
...
data1$P4_6=factor(data1$P4_6,
                   levels = c(1:3),
                   labels = c("Son pocos","Es el número adecuado", 
                              "Son demasiados"))
...
lapop1.1 = lapop1[complete.cases(lapop1$ing4), ] 
# Aquí hago que ambas variables tengan la misma cantidad de datos
```

```markdown
modelo1.1 <- lm(apoyo ~ ing4, data = lapop1.1) 
# variable dependiente, luego independiente, trabajo con data creada
summary(modelo1.1)
```
